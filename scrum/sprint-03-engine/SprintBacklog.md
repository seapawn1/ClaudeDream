...
*本节 Planning 落盘时留空，由 Product Developers 开工前填写：各条 sizing、施工顺序、技术方案细节（模块划分、错误处理、判据执行顺序等）。机械管线「不经 SDK/模型、由受信任代码直接执行」是 1.1 架构前提已定的约束，不是留给本节的 How 选择——第三节要填的是这个约束之内的组织方式。How 归 Developers，PO 与 Scrum Master 不越权。*
