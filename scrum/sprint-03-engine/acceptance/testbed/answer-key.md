...
| `4bd92cc` | chore: 记忆库入库 | 2026-07-30T10:30:00+08:00 |
...
| 文件 | 预期处置 |
|---|---|
| legacy-cron-jobs.md | 检出（H-B2）；不入索引+零链接；不得删除；预期隔离（reason M2-orphan，接口枚举已声明） |

**M3 悬空溯源（1 条）**：

| 文件 | sources 指向 | 预期处置 |
|---|---|---|
| migration-timeline-notes.md | docs/migration-notes.log | 检出（H-B3）；C3 入 C4 删有讣告；不得删除；预期隔离（reason M3-dangling-source） |
...
| `2b9acf8` | chore: 记忆库入库 | 2026-08-10T11:00:00+08:00 |
...
- **正式场**（默认阈值、默认冷却）：正式场默认阈值：在线熔断于首个超阈值点中止——明细删除 4 笔（第 4 笔触发 4 > 3，第 5 笔未及执行）；判法：报告净消失数 == 明细删除笔数（预期 4），隔离/索引修复不计入；报告写明原因/真实净消失数/回滚清单（H-G1/G2）；冷却照常起算在 CLI 直跑路径不可观察（H-G4 附发现，PO 裁）。
...
| `d916aac` | chore: 记忆库入库 | 2026-07-20T10:00:00+08:00 |
...
| `cb6019a` | chore: 记忆库入库 | 2026-07-22T10:00:00+08:00 |

## 五、G9 留话页模板

- **文件**：`out/g9-note-template.md`（builder 只产模板**不落位**）。
- **内容**：段落标记 `### User` + 隔离 slug `cache-helper-notes` + 用户原话。
- **放置**：上游梦跑完后由 verify.mjs（自动线）或 PO（手工线）放置到底片目录，并同步 ledger.json（file 字段为文件名，按 sessionId 分组）——早放会被正确实现漏检（02.6-AC1 检索窗口 = 上次梦 runId 之后）；页文件名须带 --<runId 段>（接口：页时间戳取文件名 -- 后段）。
