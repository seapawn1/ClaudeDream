...
**本目录（原 `.IDEO/`，2026-08-07 归档并入 `scrum/.IDEO/design-sprint/`）是 ClaudeDream 已结束的设计冲刺工作区**，保存挑战定义、长期目标、设计地图与方案收敛过程。这里以设计文档为主；唯一例外是原型施工物（可运行脚本、假数据 builder、真跑产物）——它们是设计迭代的道具，随所属 `Prototype-0X-*/` 子文件夹落户，与其 Sketches/Storyboard/TestPlan 同住一个迭代单元。
...
| [../../../README.md](../../../README.md) | 挑战宣言、势力范围与非目标；官方现状与两个社区事故、前人三条路径对比、原料层可行性、差异化定位——原 `ChallengeBackground.md`，2026-08-07 并入项目根 README |
...
3. Ask the Experts 的部分原始材料（决策者访谈原话、社区调研过程、40 条 HMW 原始候选）**未落盘**，是 Decider 2026-08-01 的知情选择而非遗漏——详见根 [README.md](../../../README.md)「证据与效力」节。需复查时须重跑对应模块。
