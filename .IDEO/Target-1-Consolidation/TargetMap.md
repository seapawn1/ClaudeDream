...
***S4 定位已拍板（2026-08-02，Friday Test 中裁定）**——原"机械压缩 vs LLM 提炼"之争定向 **A：机械压缩底片层**（Karpathy LLM Wiki 的 `raw/` 形制：只追加、不可变、零 API 落盘——claude-code-log 实测压缩率 1.8%；提炼留在梦内，梦是全系统唯一不占用户时间的 LLM 时刻）。compiler `flush.py` 的 SessionEnd 提炼路线（B）经对摆落选：每次收工烧 API 且判断点前移。此裁决同时是 Test 中发现的 G9 回程 bug（用户裁决无法送达下一梦）的修法：梦 D3 定向时先翻底片找用户留话。动工排产物阶段 backlog 首位。*
