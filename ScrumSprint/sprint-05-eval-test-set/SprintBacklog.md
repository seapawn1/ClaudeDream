...
| W6 | **【seapawn 操作】** 首跑 eval（Sprint 4 系统）：① 在 `F:\IDEO-Scrum` 开 Claude Code 会话跑 `/claude-dream`，完毕后 `cd ~/.claude/projects/F--IDEO-Scrum && git add -A && git commit -m "eval-run-1: IDEO-Scrum cold start"`；② 同理在 `F:\DiaryAgent` 跑一遍，完毕后 `cd ~/.claude/projects/f--DiaryAgent && git add -A && git commit -m "eval-run-1: DiaryAgent warm start"` | AC⑤ | 两个 claude project dir 均有新 commit，`git diff HEAD^` 展示完整变化 |
| W7 | 人工评分 + 撰写基线报告：对照 `test-path-matrix.md` 的预期矩阵，逐路径判断通过 / 不通过，写 `eval-report-baseline.md` | AC⑤ | 报告存在，各路径有 ✅/❌ 判定 + 通过率小结 |
